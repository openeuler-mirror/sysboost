...
mod tests {
	...
	fn test_coredump_monitor() {
		...
		let output = Command::new("gcc")
			.args(&["-o", &excute_file.to_str().unwrap(), &source_file.to_str().unwrap()])
			.output()
		...
		let output = Command::new(&excute_command).output().expect("Failed to excute command!");
	...
	fn test_bash_coredump() {
		...
		let output = Command::new("bash").arg("-c").arg("kill -s SIGSEGV $$").output().expect("Failed to excute command!");
