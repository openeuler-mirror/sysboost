...
core_dump:
	 ...
	 */
	segs = cprm->vma_count + g_sym.elf_core_extra_phdrs(cprm);
	...
	offset += g_sym.elf_core_extra_data_size(cprm);
	e_shoff = offset;
	...
	if (!g_sym.elf_core_write_extra_phdrs(cprm, offset))
		goto end_coredump;
	...
	if (!g_sym.elf_core_write_extra_data(cprm))
		goto end_coredump;
